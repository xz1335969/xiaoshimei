...
class Person:
    def __init__(self, user_id: int, group_id: int, player_id=None):
        if user_id == 0:
            ...
            if not (player_id is None):
                ...
                result = cursor.fetchone()
                if result is None:
                    sql = f'INSERT INTO user_detail (user_id,group_id,gold,rob_ban,is_superuser) VALUES ({user_id},{group_id},0,0,0)'
                    cursor2 = conn.cursor()
                    cursor2.execute(sql)
                    conn.commit()
    ...
    def refresh(self):
        ...
        sql = f'UPDATE user_detail SET gold = {self.gold} ' \
              ...
              f',rob_ban = {self.rob_ban} ' \
              f',vip_end_time = \'{self.vip_end_time.strftime("%Y-%m-%d %H:%M:%S")}\' ' \
    ...
    def refresh_all(self):
        ...
        for item in self.package.values():
            sql2 = f'UPDATE user_package SET count = {item.count} WHERE item_id = {item.item_id} AND player_id = {self.player_id}'
            sql3 = f'INSERT INTO user_package (player_id,item_id,count,type) VALUES ({self.player_id},{item.item_id},{item.count},{item.type})'
            sql4 = f"SELECT * FROM user_package WHERE player_id = {self.player_id} AND item_id = {item.item_id}"
            cursor.execute(sql4)
            result = cursor.fetchone()
            cursor = conn.cursor()
            if result is None:
...
async def gold_shop(session: CommandSession):
    ...
    player = Person(user_id, group_id)
    shop = Shop(player)
    _ = await session.aget(prompt=shop.showlist())
    _1 = _.split(' ')
    try:
        ...
        if player_choice_shop_id not in shop.shop_id_list:
            ...
            if player_choice_shop_id not in shop.show_list:
                ...
                elif item_to_buy.limit_type > 0:
                    player_choice_count = int(await session.aget(prompt=f"请输入购买数量：({lmt_txt}剩余：{rest})"))
                else:
                    player_choice_count = int(await session.aget(prompt=f"请输入购买数量："))
                if player_choice_count <= 0:
                    session.finish("请输入正确的数字！")
                if rest and player_choice_count > rest:
                    session.finish("购买数量超过上限！")
...
async def rob(session: CommandSession):
    ...
    if not arg:
        ...
        today0 = now - datetime.timedelta(seconds=now.second, minutes=now.minute, hours=now.hour)
        today0_txt = today0.strftime('%Y-%m-%d %H:%M:%S')
        sql = f"SELECT time FROM rob_log WHERE player_id= {player.player_id} AND time > TIMESTAMP('{today0_txt}')"
        conn = pymysql.connect(host='localhost', user='root', password='xz123456', database='xiaoshimei',
                               charset='utf8mb4')
        cursor = conn.cursor()
        cursor.execute(sql)
...
async def legend_box(session: CommandSession):
    ...
    try:
        ...
        if num > 300:
            session.finish("为防止服务器卡死，最大开启数量为300")
            return
        ...
        for i in result:
            ...
            box_item_list.append(item)
            box_item_dict[j] = rate
            del item
            j+=1
        empty_person = Person(0, 0)
        for i in range(num):
            item = box_item_list[overrandom(box_item_dict)]
            player.package.setdefault(item.item_id, Item(item.item_id))
            player.package[item.item_id].count += item.count
            empty_person.package.setdefault(item.item_id, Item(item.item_id))
...
@on_command("labyrinth", aliases="迷宫寻宝", only_to_me=False)
async def labyrinth(session: CommandSession):
    ...
    arg = session.current_arg.strip()
    if not arg:
        arg = await session.aget(prompt="请输入您要开启的数量：(每次500金币)")
    try:
        num = int(arg)
    except ValueError:
        session.finish("您输入的数量有误！")
...
async def simulate_fusion(session: CommandSession):
    ...
    if item_id_choice not in empty_player.package.keys():
        ...
        if _type:
            report = []
            arg = await session.aget(prompt="请输入熔炼次数（为防止刷屏，超过30次将自动折叠）")
            if arg == "全部":
                ...
                for i in range(num):
                    if item_choice.count < 4:
                        ...
                        if rand < item_choice.fusion_probability:
                            ...
                            _sum += 1
                report.append(f"熔炼完成，得到{_sum}个{item_product.name}")
            else:
                for i in range(num):
                    if item_choice.count < 4:
...
@on_command("fusion_all", only_to_me=False, aliases="批量熔炼")
async def fusion_all(session: CommandSession):
    await nonebot.command.call_command(session.bot, session.event, name="simulate_fusion", current_arg="批量")
...
@on_command("strengthen", aliases="模拟强化",only_to_me=False)
async def strengthen(session: CommandSession):
    ...
    if not len(report_list):
        session.finish("您没有能够强化的武器")
    report = '\n'.join(report_list)
    choice = int(await session.aget(prompt="请选择您要强化的武器编号：\n" + report))
    if choice not in weapon_id_list:
        ...
        if sum(stone_using) > 3:
            ...
            return
        for i,use_num in enumerate(stone_using):
            if player.package.setdefault(201+i,Item(201+i)) and player.package[201+i].count >= use_num:
                player.package[201+i].count -= use_num
            else:
                session.finish(f"您的强化石{i+1}级数量不足")
                return
        strength_stone_exp = [
            75,
            300,
...
async def item_charge(session: CommandSession):
    ...
    if len(arg) >= 3:
        ...
        player = Person(to_id, group_id)
    item = Item(item_id)
    player.package.setdefault(item_id, Item(item.item_id))
